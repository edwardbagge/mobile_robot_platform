...
def _load_floor_safe_defaults():
    ...
    return (
        ...
        all_params['wheel_velocity_controller']['ros__parameters'],
    )


def generate_launch_description():
    base_defaults, odom_defaults, controller_defaults = _load_floor_safe_defaults()
    ...
    controller_kp_pwm_per_mps = LaunchConfiguration('controller_kp_pwm_per_mps')
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                description='Stop command timeout.',
            ),
            DeclareLaunchArgument(
                'base_wheel_base_m',
                default_value=str(base_defaults['wheel_base_m']),
                description='Physical wheel base used by base/controller kinematics.',
            ),
            DeclareLaunchArgument(
                'max_wheel_speed_mps',
                default_value=str(base_defaults['max_wheel_speed_mps']),
                description='Wheel speed represented by max_pwm in control feedforward.',
            ),
            DeclareLaunchArgument(
                'max_pwm',
            ...
            DeclareLaunchArgument(
                'controller_rate_hz',
                default_value=str(controller_defaults['controller_rate_hz']),
                description='Wheel velocity controller update rate.',
            ),
            DeclareLaunchArgument(
                'controller_kp_pwm_per_mps',
                default_value=str(controller_defaults['kp_pwm_per_mps']),
                description='Proportional feedback gain from wheel speed error to PWM.',
            ),
            LogInfo(msg='Starting robot base bridge, wheel velocity controller, and odometry.'),
            LogInfo(
                msg=[
                    'Base params: max_pwm=',
                    max_pwm,
            ...
            Node(
                ...
                parameters=[
                    params_file,
                    {
                        'controller_rate_hz': ParameterValue(
                            controller_rate_hz,
                            value_type=float,
                        ),
                        'wheel_base_m': ParameterValue(base_wheel_base_m, value_type=float),
